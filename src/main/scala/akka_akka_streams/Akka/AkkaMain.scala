...
import Akka1.AkkaMain3.chnage_behaviour.WorkerProtocol
import akka.actor.typed.scaladsl.AskPattern.{Askable, schedulerFromActorSystem}
import akka.actor.typed.scaladsl.Behaviors
import akka.actor.typed.{ActorRef, ActorSystem, Behavior, Props, SpawnProtocol}
import akka.intro_actor.behaviour_factory_method
...
object  AkkaMain3_execute {
  def main(args: Array[String]): Unit ={
    val system = ActorSystem[WorkerProtocol](AkkaMain3.chnage_behaviour(), "Echo")

    system ! AkkaMain3.chnage_behaviour.WorkerProtocol.Start
    Thread.sleep(100)

    system ! AkkaMain3.chnage_behaviour.WorkerProtocol.StandBy
    Thread.sleep(100)

    system ! AkkaMain3.chnage_behaviour.WorkerProtocol.Stop
    Thread.sleep(100)
