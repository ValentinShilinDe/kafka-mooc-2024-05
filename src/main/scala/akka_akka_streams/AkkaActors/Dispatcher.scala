...
import Adapter.Dispatcher.JsonParser.{Parse, ParseResponse}
import Adapter.Dispatcher.LogWorker.{LogRequest, LogResponse}
  ...
  object TaskDispatcher{
    ...
    def apply(): Behavior[CommandDispatcher] = Behaviors.setup{ctx =>
      ...
      Behaviors.receiveMessage{
        case LogWork(msg) =>
          val logWorker: ActorRef[LogRequest] = ctx.spawn(LogWorker(), "LogWorker")
